...
Framebuffer::Framebuffer(Texture *depth, Texture **colors, size_t count)
  ...
  for (size_t i = 0; i != count; ++i) {
    ...
    glNamedFramebufferTexture(_handle.get(), GLenum(GL_COLOR_ATTACHMENT0 + i),
  ...
  ALWAYS_ASSERT(glCheckNamedFramebufferStatus(_handle.get(), GL_FRAMEBUFFER) ==
...
void Framebuffer::blit(bool depth) const {
  ...
  glBlitNamedFramebuffer(
      _handle.get(), binding, 0, 0, _size.x, _size.y, 0, 0, viewport[2],

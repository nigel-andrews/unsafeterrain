...
static std::string read_shader(const std::string &file_name,
  ...
  if (!content.is_ok) {
    FATAL((std::string("Unable to read shader: \"") + std::string(shader_path) +
           '"')
              .c_str());
  }
      ...
      if (line.substr(0, 7) == "version") {
        ...
        if (!line.empty()) {
          ...
          if (end != line.size() - 1 || delim != '"') {
            FATAL((std::string("Unable to parse shader include: \"") +
          ...
          if (includes.find(include_file) == includes.end()) {
            ...
            auto content =
                read_text_file(std::string(shader_path) + include_file);
            if (!content.is_ok) {
              FATAL((std::string("Shader include not found: \"") +
                     std::string(full_line) + '"')
                        .c_str());
            }
...
Program::Program(const std::string &comp)
    : _handle(glCreateProgram()), _is_compute(true) {
...
void Program::fetch_uniform_locations() {
  ...
  for (int i = 0; i != uniform_count; ++i) {
    ...
    glGetActiveUniform(_handle.get(), i, sizeof(name), &len, &discard, &type,
                       name);
...
std::shared_ptr<Program> Program::from_files(const std::string &frag,
...
int Program::find_location(u32 hash) {
  const auto it =
      std::lower_bound(_uniform_locations.begin(), _uniform_locations.end(),

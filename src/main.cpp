...
void process_inputs(GLFWwindow *window, Camera &camera) {
  ...
  if (glfwGetMouseButton(window, GLFW_MOUSE_BUTTON_LEFT) == GLFW_PRESS) {
    ...
    if (delta.length() > 0.0f) {
      glm::mat4 rot =
          glm::rotate(glm::mat4(1.0f), delta.x, glm::vec3(0.0f, 1.0f, 0.0f));
...
void gui(ImGuiRenderer &imgui) {
  ...
  if (ImGui::BeginMainMenuBar()) {
    ...
    if (ImGui::BeginMenu("Exposure")) {
      ImGui::DragFloat("Exposure", &exposure, 0.25f, 0.01f, 100.0f, "%.2f",
...
struct RendererState {
  static RendererState create(glm::uvec2 size) {
    ...
    if (state.size.x > 0 && state.size.y > 0) {
      ...
      state.main_framebuffer =
          Framebuffer(&state.depth_texture, std::array{&state.lit_hdr_texture});

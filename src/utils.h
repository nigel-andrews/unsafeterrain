...
namespace OM3D
{
    ...
    {
    public:
        inline ScopeGuard(T&& t)
            : _ex(FWD(t))
        {}

        inline ScopeGuard(ScopeGuard&& other)
    ...
    public:
        ...
        {}
        inline constexpr Span(T& t)
            : _data(&t)
            , _size(1)
        {}
        inline constexpr Span(T* data, size_t size)
            : _data(data)
            , _size(size)
        {}

        template <size_t N>
        inline constexpr Span(T (&arr)[N])
            : _data(arr)
            , _size(N)
        {}

        template <size_t N>
        inline constexpr Span(std::array<T, N>& arr)
            : _data(arr.data())
            , _size(N)
        {}

        template <typename C,
                  typename = std::enable_if_t<is_compat<data_type<C>>>>
        inline constexpr Span(C&& vec)
            : _data(vec.data())
            , _size(std::distance(vec.begin(), vec.end()))
        {}

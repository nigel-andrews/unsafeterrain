...
#define DEFER(expr)                                                            \
  auto CREATE_UNIQUE_NAME_WITH_PREFIX(defer) =                                 \
      ::OM3D::ScopeGuard([&]() { expr; })
...
[[noreturn]] void fatal(const char *msg, const char *file = nullptr,
                        int line = 0);
...
template <typename T> class Span {
  ...
  template <typename C> using data_type = decltype(std::declval<C>().data());
...
public:
  ...
  inline constexpr Span(C &&vec)
      : _data(vec.data()), _size(std::distance(vec.begin(), vec.end())) {}
...
template <typename T> inline constexpr void hash_combine(T &seed, T value) {
...
template <typename T> inline constexpr T to_rad(T deg) {
  return deg * T(0.01745329251994329576923690768489);
}

template <typename T> inline constexpr T to_deg(T rad) {
